...
def index_csv(self, request):
    is_admin = request.current_user and request.identity.has_global_admin_permissions
    optional_fields = TableRowOptionalFields()
    optional_fields.submitters = is_admin
    content = propositions_to_csv(self.propositions(request.q, is_admin),
